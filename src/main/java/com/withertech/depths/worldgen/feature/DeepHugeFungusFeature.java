...
public class DeepHugeFungusFeature extends Feature<DeepHugeFungusFeatureConfig>
{
	public DeepHugeFungusFeature(Codec<DeepHugeFungusFeatureConfig> codec) {
		super(codec);
	}

	public boolean generate(StructureWorldAccess structureWorldAccess, ChunkGenerator chunkGenerator, Random random, BlockPos blockPos, DeepHugeFungusFeatureConfig hugeFungusFeatureConfig) {
	...
	private void generateStem(WorldAccess world, Random random, DeepHugeFungusFeatureConfig config, BlockPos blockPos, int stemHeight, boolean thickStem) {
		...
		for(int j = -i; j <= i; ++j) {
			for(int k = -i; k <= i; ++k) {
				...
				for(int l = 0; l < stemHeight; ++l) {
					mutable.set((Vec3i)blockPos, j, l, k);
	...
	private void generateHat(WorldAccess world, Random random, DeepHugeFungusFeatureConfig config, BlockPos blockPos, int hatHeight, boolean thickStem) {
		BlockPos.Mutable mutable = new BlockPos.Mutable();
		...
		for(int k = j; k <= hatHeight; ++k) {
			...
			for(int m = -l; m <= l; ++m) {
				for(int n = -l; n <= l; ++n) {
					...
					boolean bl5 = bl2 && bl3;
					boolean bl6 = k < j + 3;
					mutable.set((Vec3i)blockPos, m, k, n);
	...
	private void generateHatBlock(WorldAccess world, Random random, DeepHugeFungusFeatureConfig config, BlockPos.Mutable pos, float decorationChance, float generationChance, float vineChance) {
